...
RSpec.describe Project, type: :model do
  describe 'validations' do
    ...
    it do
      should define_enum_for(:status).with_values([:planned, :active, :done, :failed]).with_prefix
    end

    context "owner is manager validation" do
      context "Project owner is not an employee with a manager role" do
        let(:project_record) { Project.new(name: 'new_project', owner_id: "b82522f0-8644-4c65-a552-9c6b8a9e4b7a",
                                        status: 'active' ) }
      ...
      context "Project owner is an employee with a manager role" do
        let(:project_record) { Project.new(name: 'new_project', owner_id: "f3e868b9-6151-49c4-8d51-b9e3fb9255ba",
                                          status: 'active' ) }
